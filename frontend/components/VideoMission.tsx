...
export function VideoMission() {
  ...
  async function analyzeVideo() {
    ...
    try {
      ...
      if (!checkpoint) { setError('No tasks found in this video.'); return; }
    ...
    <>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-on-surface-variant mt-0.5">
              Paste a YouTube link — Gemini generates tasks, you write code, earn XP.
            </p>
          </div>
          {totalXp > 0 && (
          ...
          <div className="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 overflow-hidden">
            <div className="px-8 pt-8 pb-4 border-b border-outline-variant/10">
              ...
              <p className="text-sm text-on-surface-variant mt-1">
                Write code for each task — Gemini reviews your solution and awards XP.
              </p>
            </div>
              ...
              <div className="mx-8 mb-8 p-6 bg-primary/5 rounded-lg flex items-center gap-4 border border-primary/10">
                ...
                <div>
                  ...
                  <p className="text-sm text-on-surface-variant">
                    You earned <span className="font-bold text-primary">+{totalXp} XP</span> from this video.
                  </p>
                </div>
                <button
                  onClick={() => { setPlan(null); setUrl(''); setCompletedTasks(new Set()); setTotalXp(0); }}
      ...
      {editorOpen && plan && (
        <CodeEditor
          taskKey={editorOpen}
          taskTitle={
            editorOpen === 'task1' ? plan.task1.title :
            editorOpen === 'task2' ? plan.task2.title :
            plan.practice.title
          }
          taskDescription={
            editorOpen === 'task1' ? plan.task1.description :
            editorOpen === 'task2' ? plan.task2.description :
            plan.practice.description
          }
          planId={plan.plan_id}
          dayNumber={1}
